...
define('SUC04', '登録しました');
define('SUC05', '購入しました！店舗と連絡を取りましょう！');
define('SUC06', '体育館の予約が完了しました');
//================================
// グローバル変数
//================================
//エラーメッセージ格納用の配列
...
function dbConnect(){
  //DBへの接続準備
  $dsn = 'mysql:dbname=badminton;host=localhost;charset=utf8';
  $user = 'root';
  $password = 'root';
  $options = array(
...
function getMyBuyProducts($u_id){
  debug('自分の商品情報を取得します。');
  debug('ユーザーID：'.$u_id);
  //例外処理
  try {
    ...
    $dbh = dbConnect();
    // SQL文作成
    $sql = 'SELECT * FROM bord WHERE buy_user = :buy_user_id AND delete_flg = 0';
    $data = array(':buy_user_id' => $u_id);
    // クエリ実行
    $stmt = queryPost($dbh, $sql, $data);
...
function getProduct($user_id,$_product_id){
  debug('商品情報を取得します');
  ...
  try{
    ...
    $dbh = dbConnect();
    //SQL文作成
    $sql = 'SELECT * FROM product WHERE user_id = ::user_id AND id = :product_id AND delete_flg = 0';
...
function getProductList($currentMinNum = 1, $category, $sort, $span = 15){
  debug('商品情報を取得します');
  //例外処理
  try{
    // DBへ接続
    ...
    $data = array();
    //クエリ実行
    $stmt = queryPost($dbh, $sql, $data);//総レコード数
